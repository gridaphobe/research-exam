...
*** QuickCheck
QuickCheck \cite{claessen_quickcheck:_2000} enables randomized testing of
Haskell programs by providing an embedded domain-specific language for
generating /arbitrary/ values of a given datatype. As with SmallCheck,
QuickCheck properties are boolean-valued Haskell functions whose inputs can be
(randomly) generated. Unlike SmallCheck it is impractical to automatically
...
to generate lists with more than a handful of elements.
